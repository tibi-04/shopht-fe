...
export const AdminEditProduct = ({ onClose, productData, fetchdata }) => {
  ...
  const handleDeleteMoreDetail = (index) => {
    const newMoreDetails = [...data.more_details];
    newMoreDetails.splice(index, 1);
    setData((prev) => ({
      ...prev,
      more_details: newMoreDetails,
    }));
        ...
        >
          ...
          <div className="space-y-2">
            ...
            {data.more_details.length > 0 ? (
              data.more_details.map((detail, index) => (
                <div key={index} className="flex gap-2 items-center">
                  ...
                  <button
                    type="button"
                    onClick={() => handleDeleteMoreDetail(index)}
              ...
              <p className="text-gray-500 italic">Chưa có chi tiết thêm</p>
            )}

            <button
              type="button"
              onClick={handleAddMoreDetail}
              className="mt-2 px-4 py-2 bg-[#A12B58] text-white rounded hover:bg-[#8a2450]"
            >
              Thêm chi tiết
            </button>
          </div>
